...
- `V2K-T3` — Doutrina do enforcement em §9 + entrada no `guardrails-check` + bump `1.3.0` — [Opus] — done *(`C-01`c; fecha o Bloco A de enforcement executável)*
  ...
  - Atenção do dono: a tag `kit-v1.3.0` foi criada sobre o `HEAD` atual (`d6ffe0e`), que ainda **não** contém as mudanças do Bloco A (working tree com `V2K-T2` + `V2K-T3` não commitadas, por instrução). Ao commitar, mover a tag para o commit certo (`git tag -f -a kit-v1.3.0`) antes de qualquer push.
